...
public class AudioMachine {
    ...
    public static double[] runOneGame(String game_file, String level_file, boolean visuals, String agentNames,
		...
		for (int i = 0; i < no_players; i++) {
			...
			if (no_players > 1) {
			// multi player
			players[i] = AudioMachine.createMultiPlayer(names[i], actionFile, toPlay.getObservationMulti(i),
				randomSeed, i, humans[i]);
			} else {
			// single player
			players[i] = AudioMachine.createPlayer(names[i], actionFile, toPlay.getObservation(), randomSeed,
				humans[i]);
			}
		...
		AudioMachine.tearPlayerDown(toPlay, players, actionFile, randomSeed, true);
    ...
    public static double[] replayGame(String game_file, String level_file, boolean visuals, String actionFile) {
		...
		for (int i = 0; i < no_players; i++) {
			if (no_players > 1) {
			// multi player
			players[i] = AudioMachine.createMultiPlayer(agentName, null, toPlay.getObservationMulti(i), -1, i,
				false);
			} else {
			// single player
			players[i] = AudioMachine.createPlayer(agentName, null, toPlay.getObservation(), -1, false);
			}
		...
		AudioMachine.tearPlayerDown(toPlay, players, actionFile, seed, false);
	...
	for (String level_file : level_files) {
		...
		for (int j = 0; j < no_players; j++) {
		    if (no_players > 1) {
			// multi player
			players[j] = AudioMachine.createMultiPlayer(agentNames[j], filename,
				toPlay.getObservationMulti(i), randomSeed, j, false);
		    } else {
			// single player
			players[j] = AudioMachine.createPlayer(agentNames[j], filename, toPlay.getObservation(),
				randomSeed, false);
		    }
		    score[j] = -1;
		    if (players[j] == null) {
		...
		// down.
		if (!AudioMachine.tearPlayerDown(toPlay, players, filename, randomSeed, true)) {
		    score = toPlay.handleResult();
		    toPlay.printResult();
		}
